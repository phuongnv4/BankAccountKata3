...
public class TransactionDAO {
	...
	public List<TransactionDTO> get(String accountNumber) {
		Cursor cursor = db.query(DBHelper.TABLE_TRANSACTION, new String[] {
				...
				DBHelper.ACCOUNT_NUMBER + "=?", new String[] { accountNumber },
				null, null, null);
		return paserCursor(cursor);
	}

	//
	public List<TransactionDTO> get(String accountNumber, Long startTime,
			Long stopTime) {
	...
	private List<TransactionDTO> paserCursor(Cursor cursor) {
		...
		if (cursor.moveToFirst()) {
			do {
				...
				transactionDTO.setDescription(cursor.getString(cursor
						.getColumnIndex(DBHelper.DESCRIPTION)));
				transactionDTO.setTimestamp(cursor.getLong(cursor
